...
function economyChangeSummary(tradeActivity) {
    ...
    for (let i = 0; i < tradeActivity.length; i++) {
        for (let j = 0; j < tradeActivity[i].length; j++) {
            let match = tradeActivity[i][j].match(/^(.+?)([+-])(\d+)%$/);
            
            let hasil = {}
            let currentJeff = duitJeff
            let currentLarry = duitLarry
            let currentJack = duitJack

            hasil["nama"] = match[1] 

            if (match[1] === "Jeff Bezos") {
                if (match[2] === "+") {
                    hasil["deposit"] = parseInt(match[3])/100 * currentJeff + currentJeff
                } else {
                    hasil["deposit"] = currentJeff - (parseInt(match[3])/100 * currentJeff ) 
                }
                hasil["Owner"] = "Amazon"
                duitJeff = hasil.deposit
                activity.push(hasil)
            } else if (match[1] === "Larry Page") {
                if (match[2] === "+") {
                    hasil["deposit"] = parseInt(match[3])/100 * currentLarry + currentLarry
                } else {
                    hasil["deposit"] = currentLarry - (parseInt(match[3])/100 * currentLarry ) 
                }
                hasil["Owner"] = "Google"
                duitLarry = hasil.deposit
                activity.push(hasil)
            } else {
                if (match[2] === "+") {
                    hasil["deposit"] = parseInt(match[3])/100 * currentJack + currentJack
                } else {
                    hasil["deposit"] = currentJack - (parseInt(match[3])/100 * currentJack ) 
                }
                hasil["Owner"] = "Alibaba"
                duitJack = hasil.deposit
                activity.push(hasil)
            }
        }
    }
