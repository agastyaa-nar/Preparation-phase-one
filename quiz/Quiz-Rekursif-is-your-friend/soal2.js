...
function changeXRecursive(data, jenis) {
    ...
    let hurufPertama = data[0];
    let cekJenis = parseInt(hurufPertama) % 2 === 0;

    if(jenis === "genap" && !cekJenis || jenis === "ganjil" && cekJenis) {
        hurufPertama = "x";
    }

    return hurufPertama + changeXRecursive(data.slice(1), jenis);
  }
